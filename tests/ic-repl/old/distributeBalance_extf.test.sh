...
call extf.balance(record { token = token_identifier; user = variant { "principal" = cycles_dispenser }});
assert _ == variant { ok = 0 : nat };

// Test that the command fails if the cycles_dispenser does not have any extf token
