...
// Test that the command fails if the nft does not belong to the cycles_provider
...
call dip721.transfer(cycles_provider, nft_identifier);
//assert _ == variant { ok };

call dip721.tokenMetadata(nft_identifier);
//assert _.ok.owner == opt cycles_provider;
