...
let toPowerUp1 = installToPowerUp(cycles_dispenser, 0);
...
call cycles_dispenser.distributeCycles();
assert _ == false;
...
walletReceive(default_wallet, cycles_dispenser, 200_000_000);
call cycles_dispenser.cyclesBalance();
...
let toPowerUp2 = installToPowerUp(cycles_dispenser, 0);
call cycles_dispenser.configure(variant {AddAllowList = record {
  canister = toPowerUp2;
...
walletReceive(default_wallet, cycles_dispenser, 300_000_000);
call cycles_dispenser.cyclesBalance();
...
let toPowerUp3 = installToPowerUp(cycles_dispenser, 300_000_000);
call cycles_dispenser.configure(variant {AddAllowList = record {
  canister = toPowerUp3;
  balance_threshold = 200_000_000;
  balance_target = 400_000_000;
