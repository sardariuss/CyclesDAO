...
call cycles_provider.configure(variant {SetCycleExchangeConfig = valid_cycles_config});
assert _ == variant { ok };
...
call cycles_provider.configure(variant {SetCycleExchangeConfig = invalid_cycles_config});
