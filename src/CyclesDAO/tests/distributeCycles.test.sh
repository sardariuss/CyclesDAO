...
let toPowerUp1 = installToPowerUp(cyclesDao, 0);
...
walletReceive(default_wallet, cyclesDao, 200_000_000);
call cyclesDao.cyclesBalance();
...
let toPowerUp2 = installToPowerUp(cyclesDao, 0);
call cyclesDao.configure(variant {AddAllowList = record {
  canister = toPowerUp2;
...
walletReceive(default_wallet, cyclesDao, 300_000_000);
call cyclesDao.cyclesBalance();
